...
// 1. Create a "Mock API" inside the test to verify Athena's behavior
const app = express();
...
app.put('/api/users/:id', (req, res) => {
  const auth = req.headers.authorization;
  // Verify that Athena correctly injected the NEW user's token
...
test('Unit Test: User Lifecycle via Mock App', async () => {
  ...
  const athena = request(app);

  // Step 2: Create User
  const createRes = await athena
    .as(superuserToken)
    .post('/api/users')
    .send(createPayload);
